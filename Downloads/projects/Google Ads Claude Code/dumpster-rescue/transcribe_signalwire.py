...
EXTRACT_SYSTEM = (
    ...
    "caller_name (the CUSTOMER's name ONLY, if THEY state it, e.g. 'Maria','Dan Moore'; null otherwise. "
    "  The business/owner ANSWERS these calls and introduces himself — e.g. 'thanks for calling Dumpster Rescue, this is Frank' — "
    "  so NEVER use the name of the person ANSWERING or the company (especially 'Frank', the owner) as caller_name. "
    "  That is the business, not the customer. If the customer never gives their own name, return null.), "
    ...
    "price (number in USD or null): the most-likely amount the customer would pay. "
    "  If ANY dollar figure was quoted on the call — a single price, a RANGE or several options "
    "  (e.g. '$450 rental, $800 if we load, $995 for a 20-yard'), or 'depends how much we fill' — return a "
    "  realistic ESTIMATE EVEN IF the caller did NOT commit, agree, or set a date. Pick the most-likely "
    "  option and lean LOW; NEVER default to the highest. A 'quoted, no firm date' call should STILL have a "
    "  price (use the figure the rep gave, leaning to the lower/partial end if the caller hedged on scope). "
    "  Set null ONLY when NO dollar amount was mentioned on the call at all. "
...
EXTRACT_SYSTEM_CONCRETE = (
    ...
    "caller_name (the CUSTOMER's name ONLY, if THEY state it, e.g. 'Maria','Dan Moore'; null otherwise. "
    "  The business/owner ANSWERS these calls and introduces himself (e.g. 'thanks for calling, this is Bruce') — "
    "  NEVER use the name of the person ANSWERING or the company as caller_name; that is the business, not the customer. "
    "  If the customer never gives their own name, return null.), "
    ...
    "service_city (the city/town the job is in, or null), "
    "price (number in USD or null): the most-likely amount the customer would pay. "
    "  If ANY dollar figure was quoted — a single price, a RANGE, or per-square-foot pricing — return a "
    "  realistic ESTIMATE EVEN IF the caller did NOT commit, agree, or set a date. Pick the most-likely "
    "  option and lean LOW; NEVER default to the highest. A 'quoted, no firm date' call should STILL have a "
    "  price (if priced per sq ft with no firm total, estimate the most likely actual charge, lean LOW). "
    "  Set null ONLY when NO dollar amount was mentioned on the call at all. "
    ...
    "  driveway ~$3,000-9,000; foundations/commercial can run higher). "
    "price_firm (bool): true ONLY if ONE specific dollar price was clearly agreed (not a range/estimate). "
    "spam (bool): true only when status=='spam'. A REAL prospective customer is NOT spam even if they reached "
